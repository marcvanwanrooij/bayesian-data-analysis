...
fname = fullfile('..','..','DAC','LocalizationPoolData.mat'); % file name, relative path
load(fname);
...
samples = jags_anova3(Y,X1,X2,X3);
...
for ii = 1:n
	mu = mean(b1(:,ii))+b0;
	hdi = hdimcmc(b1(:,ii)+b0);
	errorbar(u(ii)+1,mu,mu-hdi(1),hdi(2)-mu,'o','MarkerSize',20,'LineWidth',2);
end
nicegraph;
...
for ii = 1:n
	mu = mean(b2(:,ii))+b0;
	hdi = hdimcmc(b2(:,ii)+b0);
	errorbar(u(ii)+0.1,mu,mu-hdi(1),hdi(2)-mu,'o','MarkerSize',20,'LineWidth',2);
end
nicegraph;
...
clf
subplot(121)
plotpost(b1(:,1)-b1(:,11),'xlim',[-10 10]);

subplot(122)
plotpost(b2(:,1)-b2(:,3),'xlim',[-3 3]);
...
for ii = 1:n
	for jj = 1:3
	mu = mean(b1(:,ii))+mean(b2(:,jj))+mean(squeeze(b1b2(:,ii,jj)))+b0;
	hdi = hdimcmc(squeeze(b1b2(:,ii,jj))+b0);
	errorbar(u(ii)+1,mu,hdi(1),hdi(2),'o','MarkerSize',20,'LineWidth',2);
	end
end
nicegraph;
%%
return
s = data(:,8); % subject number
